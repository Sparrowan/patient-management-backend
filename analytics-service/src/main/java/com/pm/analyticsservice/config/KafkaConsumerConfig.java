...
/**
 * Consumer resilience for the {@code patient-events} listener: retry a few times, then route the
 * record to a dead-letter topic ({@code patient-events.DLT}) instead of blocking the partition on a
 * poison message. Boot wires this {@link DefaultErrorHandler} into the listener container (it's the
 * single {@code CommonErrorHandler} bean).
 *
 * <p>Two DLT producers because a failed record's value has one of two shapes: a valid Avro object (a
 * <b>processing</b> failure) → {@link KafkaAvroSerializer}; or raw bytes (a <b>deserialization</b>
 * failure — schema/poison) → {@link ByteArraySerializer}. {@link DeadLetterPublishingRecoverer}
 * routes to the right template by the value's runtime type.
 */
@Configuration
