...
class PatientControllerTest {
    ...
    @Test
    @DisplayName("GET /patients returns 200 with a paginated envelope")
    ...
    @Test
    @DisplayName("GET /patients/{id} returns 200 when found")
    ...
    @Test
    @DisplayName("GET /patients/{id} returns 404 ProblemDetail when absent")
    ...
    @Test
    @DisplayName("POST /patients returns 201 for a valid body")
    ...
    @Test
    @DisplayName("POST /patients returns 400 with field errors for an invalid body")
    void createReturns400() throws Exception {
        String invalid = """
                {"name":"","email":"not-an-email","address":"x","dateOfBirth":"2999-01-01"}""";

        mockMvc.perform(post("/patients").contentType(MediaType.APPLICATION_JSON).content(invalid))
    ...
    @Test
    @DisplayName("POST /patients returns 409 for a duplicate email")
    ...
    @Test
    @DisplayName("PUT /patients/{id} returns 200 for a valid update")
    void updateReturns200() throws Exception {
        ...
        mockMvc.perform(put("/patients/{id}", ID).contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.version").value(1));
    }

    @Test
    @DisplayName("PUT /patients/{id} returns 400 when version is missing")
    ...
    @Test
    @DisplayName("PUT /patients/{id} returns 409 on a stale version")
    ...
    @Test
    @DisplayName("DELETE /patients/{id} returns 204")
    void deleteReturns204() throws Exception {
        mockMvc.perform(delete("/patients/{id}", ID)).andExpect(status().isNoContent());
    }
}
